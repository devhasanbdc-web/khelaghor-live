...
export default function App() {
  ...
  const processedChannels = useMemo(() => {
    ...
    // Auto-sort to guarantee LIVE / Sports games and premium television streams are ALWAYS at the top
    return [...list].sort((a, b) => {
      const getScore = (ch: typeof a) => {
        let score = 0;

        // 0. FIFA World Cup channels get the absolute highest priority — always pinned to the top
        if (ch.isFifa) {
          score += 50000;
        }
        ...
        // 3. Premium targeted live sports channels (like BTV, T Sports, Sony, Star)
  ...
  const hotLiveChannels = useMemo(() => {
    // Highly active channels that represent HOT sports matching user's specific guidelines
        ...
        {loading ? (
          ...
          <>
                        ...
                        <div className="flex gap-2 mt-3 w-full justify-center flex-wrap">
                          {footballChannels.slice(0, 4).map((ch) => (
                            <button
                              ...
                              className="px-3 py-1.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-350 hover:text-white rounded-lg text-[10px] font-bold transition flex items-center gap-1.5 cursor-pointer"
                            >
                              <span>⚽ {ch.name}</span>
                            </button>
                          ))}
            ...
            <div className="space-y-4 pt-4 border-t border-zinc-900">
              ...
              {/* FIFA WORLD CUP SCHEDULE & LIVE TRACKER DECK */}
              <div className="bg-gradient-to-b from-zinc-950 to-zinc-900 border border-amber-500/15 p-6 rounded-3xl mt-6 relative shadow-lg overflow-hidden">
                <div className="absolute top-0 right-0 h-40 w-40 bg-amber-500/5 blur-3xl rounded-full" />
                
                {/* Header info */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 pb-4 border-b border-zinc-900 relative z-10">
                  <div>
                    <div className="flex items-center gap-2">
                      <Flame className="w-5 h-5 text-amber-500 animate-pulse" />
                      <h3 className="text-sm font-black uppercase text-amber-400 tracking-wider">
                        {isBengali ? "ফিফা বিশ্বকাপ সরাসরি সময়সূচী ও লাইভ স্কোর" : "FIFA World Cup Live Fixtures & Real-time Scores"}
                      </h3>
                    </div>
                    <p className="text-[10px] text-zinc-400 font-sans mt-1">
                      {isBengali ? "Gemini Live Grounding দ্বারা সরাসরি সংগৃহীত ম্যাচসমূহ, যা প্রতি ১ মিনিটে স্বয়ংক্রিয়ভাবে আপডেট হয়" : "Live real-time matches sourced dynamically via Gemini, auto-refreshing every minute"}
                    </p>
                  </div>
                  
                  <div className="flex items-center gap-2.5">
                    {/* Manual fixtures refresh button */}
                    <button
                      id="manual-fixtures-reload"
                      onClick={() => fetchFifaMatches(true)}
                      disabled={loadingMatches}
                      className="p-1.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-400 hover:text-white rounded-xl transition duration-150 disabled:opacity-50 flex items-center justify-center cursor-pointer"
                      title={isBengali ? "স্কোরবোর্ড রিফ্রেশ করুন" : "Refresh scoreboard"}
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${loadingMatches ? 'animate-spin text-amber-500' : ''}`} />
                    </button>

                    {/* Total stats */}
                    <span className="text-[10px] font-mono font-bold bg-amber-500/10 text-amber-400 border border-amber-500/20 px-3 py-1.5 rounded-xl flex items-center gap-1.5">
                      <span className="h-2 w-2 rounded-full bg-red-400 animate-pulse" />
                      {fifaMatches.filter(m => m.status === 'live').length} {isBengali ? "টি লাইভ ম্যাচ" : "Live Match Active"}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3 relative z-10">
                  {loadingMatches && sortedFifaMatches.length === 0 ? (
                    // Beautiful loader skeletons representing premium live streams being parsed
                    <div className="space-y-3">
                      {[1, 2, 3].map((n) => (
                        <div key={n} className="p-4 rounded-2xl border border-zinc-900 bg-zinc-950 animate-pulse flex flex-col lg:flex-row items-center justify-between gap-4">
                          ...
                          <div className="h-7 w-20 bg-zinc-800 rounded-xl animate-pulse" />
                        </div>
                      ))}
                    </div>
                  ) : sortedFifaMatches.length === 0 ? (
                    <div className="p-8 text-center bg-zinc-900/10 border border-zinc-850 rounded-2xl flex flex-col items-center justify-center">
                      <Trophy className="w-8 h-8 text-zinc-600 mb-2 animate-bounce" />
                      <h4 className="text-xs text-zinc-350 font-black">{isBengali ? "কোনো ম্যাচ পাওয়া যায়নি" : "No Real-time Matches Sourced"}</h4>
                      <p className="text-[10px] text-zinc-500 mt-1 max-w-sm">
                        {isBengali ? "এই সেকেন্ডে কোনো আন্তর্জাতিক ফুটবল ম্যাচ সংযুক্ত নেই। দয়া করে রিফ্রেশ বাটনে চাপ দিয়ে পুনরায় চেষ্টা করুন।" : "No matches fetched at this second. Try pressing the update button above to pull latest schedules!"}
                      </p>
                      <button
                        id="empty-state-fetch"
                        type="button"
                        onClick={() => fetchFifaMatches(true)}
                        className="mt-3.5 px-3 py-1.5 bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 text-[10px] font-black text-amber-400 hover:text-amber-300 rounded-lg transition shrink-0 cursor-pointer"
                      >
                        {isBengali ? "পুনরায় লোড করুন" : "Reload Matches"}
                      </button>
                    </div>
                  ) : (
                    sortedFifaMatches.map((match) => {
                      const isMatchLive = match.status === 'live';
                      const isMatchCompleted = match.status === 'completed';
                    
                    return (
                      <div
                        id={`match-schedule-card-${match.id}`}
                        key={match.id}
                        className={`p-4 rounded-2xl border transition-all duration-300 relative overflow-hidden ${
                          isMatchLive
                            ? 'bg-gradient-to-r from-red-500/10 via-amber-500/5 to-zinc-950 border-red-500/55 shadow-md shadow-red-500/5'
                            : isMatchCompleted
                              ? 'bg-zinc-900/25 border-zinc-800/40 opacity-75'
                              : 'bg-zinc-900/40 hover:bg-zinc-900 border-zinc-900'
                        }`}
                      >
                        {/* Background subtle glowing bar for live matches */}
                        {isMatchLive && (
                        ...
                        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center justify-between lg:justify-start gap-3">
                            <span className="text-[10px] font-mono font-extrabold uppercase bg-zinc-850 text-zinc-300 px-2.5 py-1 rounded-lg border border-zinc-805">
                              {isBengali ? match.matchTypeBn : match.matchType}
                            </span>
                            
                            {/* Live HOT / Status indicator */}
                            {isMatchLive ? (
                              <span className="flex items-center gap-1 bg-red-600/90 text-white font-black text-[9px] px-2.5 py-1 rounded-full uppercase tracking-wider animate-pulse shadow-md shadow-red-600/20">
                                <Flame className="w-3 h-3 animate-bounce" />
                                {isBengali ? "হট লাইভ 🔴" : "HOT LIVE 🔴"}
                              </span>
                            ) : isMatchCompleted ? (
                              <span className="text-zinc-500 font-bold text-[9px] uppercase tracking-wider bg-zinc-800/30 px-2.5 py-1 rounded-full border border-zinc-800">
                                {isBengali ? "শেষ হয়েছে" : "Completed"}
                              </span>
                            ) : (
                              <span className="text-amber-400 font-bold text-[9px] uppercase tracking-wider bg-amber-500/10 px-2.5 py-1 rounded-full border border-amber-500/15">
                                {isBengali ? "আসন্ন ম্যাচ" : "Upcoming"}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center justify-center gap-6 py-2">
                            {/* Home Team */}
                            <div className="flex items-center gap-2.5 text-right w-36 justify-end">
                              <span className="text-xs font-black text-zinc-150">{isBengali ? match.homeTeamBn : match.homeTeam}</span>
                              <span className="text-2xl w-8 h-8 flex items-center justify-center bg-zinc-950 rounded-full border border-zinc-900 shadow-inner flex-shrink-0" title={match.homeTeam}>{match.homeFlag}</span>
                            </div>

                            {/* Center Score or VS */}
                            <div className="flex flex-col items-center justify-center min-w-[70px]">
                              {isMatchLive || isMatchCompleted ? (
                                <div className="flex items-center gap-2 bg-zinc-950 px-3 py-1 rounded-xl border border-zinc-900 shadow-md">
                                  <span className={`text-sm font-black ${isMatchLive ? 'text-red-400 animate-pulse' : 'text-zinc-400'}`}>{match.homeScore}</span>
                                  <span className="text-zinc-650 text-[10px] font-bold">-</span>
                                  <span className={`text-sm font-black ${isMatchLive ? 'text-red-400 animate-pulse' : 'text-zinc-400'}`}>{match.awayScore}</span>
                                </div>
                              ) : (
                                <span className="text-xs font-black text-zinc-500 uppercase">VS</span>
                              )}
                              
                              {/* Minute or kickoff date indicator */}
                              <span className="text-[9px] font-mono font-extrabold text-amber-500 mt-1 max-w-[100px] text-center truncate">
                                {isMatchLive ? (isBengali ? `${match.timeRemainingBn} মিনিট` : `${match.timeRemaining} MIN`) : (isBengali ? match.dateTimeBn : match.dateTime)}
                              </span>
                            </div>

                            {/* Away Team */}
                            <div className="flex items-center gap-2.5 text-left w-36">
                              <span className="text-2xl w-8 h-8 flex items-center justify-center bg-zinc-950 rounded-full border border-zinc-900 shadow-inner flex-shrink-0" title={match.awayTeam}>{match.awayFlag}</span>
                              <span className="text-xs font-black text-zinc-150">{isBengali ? match.awayTeamBn : match.awayTeam}</span>
                            </div>
                          </div>

                          {/* Quick Streaming / Simulated Actions */}
                          <div className="flex items-center justify-end gap-2">
                            {/* Simulated controller allowing user to fluctuate status in realtime */}
                            <div className="flex items-center gap-1 bg-zinc-950 p-1 rounded-xl border border-zinc-900">
                              <span className="text-[8px] font-extrabold text-zinc-500 uppercase px-1.5 block lg:hidden xl:block">
                                {isBengali ? "সিমুলেট:" : "Sim:"}
                              </span>
                              <button
                                id={`set-live-${match.id}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setFifaMatches(prev => prev.map(m => {
                                    if (m.id === match.id) {
                                      return { ...m, status: 'live', homeScore: m.homeScore ?? 2, awayScore: m.awayScore ?? 1, timeRemaining: "78'", timeRemainingBn: "৭৮'" };
                                    }
                                    return m;
                                  }));
                                }}
                                className={`text-[8px] font-black px-1.5 py-1 rounded transition duration-150 ${
                                  isMatchLive ? 'bg-red-500 text-white' : 'hover:bg-zinc-800 text-zinc-450'
                                }`}
                              >
                                {isBengali ? "লাইভ" : "Live"}
                              </button>
                              <button
                                id={`set-upcoming-${match.id}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setFifaMatches(prev => prev.map(m => {
                                    if (m.id === match.id) {
                                      const { homeScore, awayScore, timeRemaining, timeRemainingBn, ...rest } = m;
                                ...
                                className={`text-[8px] font-black px-1.5 py-1 rounded transition duration-150 ${
                                  match.status === 'upcoming' ? 'bg-amber-500 text-zinc-950' : 'hover:bg-zinc-800 text-zinc-450'
                                }`}
                              >
                                {isBengali ? "আসন্ন" : "Sched"}
                              </button>
                              <button
                                id={`set-completed-${match.id}`}
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setFifaMatches(prev => prev.map(m => {
                                    if (m.id === match.id) {
                                      return { ...m, status: 'completed', homeScore: m.homeScore ?? 3, awayScore: m.awayScore ?? 2 };
                                    }
                                    return m;
                                  }));
                                }}
                                className={`text-[8px] font-black px-1.5 py-1 rounded transition duration-150 ${
                                  isMatchCompleted ? 'bg-blue-600 text-white' : 'hover:bg-zinc-800 text-zinc-450'
                                }`}
                              >
                                {isBengali ? "শেষ" : "Final"}
                              </button>
                            ...
                            {/* Watch Now Call to Action (if live, play stream) */}
                            {isMatchLive ? (
                              <button
                                id={`watch-match-${match.id}`}
                                onClick={() => {
                                  const sportsCh = fifaChannels[0] || footballChannels[0] || processedChannels[0];
                                  if (sportsCh) {
                                    setSelectedChannel(sportsCh);
                                    window.scrollTo({ top: 120, behavior: 'smooth' });
                                  }
                                }}
                                className="px-3 py-1.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-zinc-950 font-black text-[10px] rounded-xl flex items-center gap-0.5 shadow-md shadow-amber-500/10 hover:scale-105 active:scale-95 transition cursor-pointer"
                              >
                                <span>{isBengali ? "খেলা দেখুন" : "Watch"}</span>
                                <ChevronRight className="w-3 h-3 text-zinc-950" />
                              </button>
                            ) : (
                              <button
                                id={`sc-upcoming-${match.id}`}
                                disabled
                                className="px-3 py-1.5 bg-zinc-800 text-zinc-500 font-extrabold text-[10px] rounded-xl border border-zinc-750 cursor-not-allowed opacity-60"
                              >
                                {match.dateLabel}
                              </button>
                            )}
